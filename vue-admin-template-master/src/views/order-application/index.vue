  ...
  <div class="order-application">
    ...
    >
      ...
      <el-table-column align="center" label="操作" width="200">
        <template slot-scope="scope">
          <el-button style="margin-right: 10px" type="success" @click="handleOpen(scope.row.id)">接单</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      return request({
        url: '/business_admin/order-application',
        method: 'post',
        data: {
          userId: this.token
        }
    ...
    updateList(id) {
      return request({
        url: '/business_admin/order-application/update',
        method: 'post',
        data: {
          id
        }
    ...
    async handleOpen(id) {
      const { data } = await this.updateList(id)
      if (data.code === 200) {
        this.$message({
